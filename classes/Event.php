...
class Event extends ElggObject {
	...
	public function generateNewAttendee() {
		...
		if ($waiting_user = $this->getFirstWaitingUser()) {
			...
			if ($rsvp) {
				...
				notify_user(elgg_get_logged_in_user_guid(), $this->getGUID(), elgg_echo('event_manager:event:registration:notification:user:subject'), elgg_echo('event_manager:event:registration:notification:user:text:event_spotfree', array(
					$waiting_user->name,
					$this->getURL(),
					$this->title)
				));
	 ...
	 * Get Event Speakers
	 */
	public function getEventSpeakers($options = array()) {

		$default = array(
			'type' => 'object',
			'subtype' => EventSpeaker::SUBTYPE,
	 ...
	 * List Event Speakers
	 */
	public function listEventSpeakers($options = array()) {
	...
	/**
	 * Convert the entity url to be shared into social sites and then can be tracked
	 * 
	 * @param string $source the source type, for example when is shared into twitter or facebook
	 * 
	 * @return string
	 */
	public function getSocialShareUrl($source) {
		$url = $this->getURL();
		if (!is_string($source) || !$source) {
			return $url;
		}

		return elgg_http_add_url_query_elements($url, array('source' => $source, 'source_type' => 'social'));
	}
